...
const ModalFilm = (props: IProps) => {
  ...
  const [dataImages, setDataImages] = useState<IFilmImages[]>([]);
  ...
  const submitFilm = async (values: any) => {
    ...
    if (dataImages.length === 0) {
      message.error("Vui lòng upload images");
      return;
    }
    ...
    if (dataInit?._id) {
      ...
      const film = {
        ...
        images: dataImages.map((item) => item.name),
  ...
  const handleRemoveFiles = (file: any) => {
    setDataImages([]);
  };
                  ...
                  rules={[
                    {
                      ...
                      message: "Vui lòng không bỏ trống",
                      validator: () => {
                        if (dataImages.length > 0) return Promise.resolve();
                        else return Promise.reject(false);
                      },
                    },
                  ]}
                >
                  <ConfigProvider locale={enUS}>
                    <Upload
                      ...
                      beforeUpload={beforeUpload}
                      onChange={handleChange}
                      onRemove={(files) => handleRemoveFiles(files)}

import { callUpdateTicketStatus } from "@/config/api";
...
interface IProps {
    ...
    open: boolean;
    dataInit: ITicket | null | any;
    setDataInit: (v: any) => void;
    reloadTable: () => void;
}
const ViewDetailTicket = (props: IProps) => {
    ...
    const handleChangeStatus = async () => {
        ...
        const res = await callUpdateTicketStatus(dataInit?._id, status)
        if (res.data) {
            message.success("Update Ticket status thành công!");
            setDataInit(null);
            onClose(false);
            reloadTable();
        } else {
            ...
            >
                <Descriptions title="" bordered column={2} layout="vertical">
                    ...
                    <Descriptions.Item label="Tên phim">
                        {dataInit?.filmId?.name}

                    </Descriptions.Item>
                    <Descriptions.Item label="Tên phòng">
                        {dataInit?.roomId?.name}
                    </Descriptions.Item>

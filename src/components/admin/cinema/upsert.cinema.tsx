...
interface ICinemaLogo {
    name: string;
    uid: string;
}

const ModalCinema = (props: IProps) => {
    ...
    const [dataLogo, setDataLogo] = useState<ICinemaLogo[]>([]);
    ...
    const submitCinema = async (valuesForm: ICinemaForm) => {
        ...
        if (dataInit?._id) {
            //update
            const res = await callUpdateCinema(dataInit._id, name, address, value, dataLogo[0].name);
            if (res.data) {
            ...
            //create
            const res = await callCreateCinema(name, address, value, dataLogo[0].name);
            if (res.data) {
                message.success("Thêm mới cinema thành công");
                handleReset();
                reloadTable();
            } else {
                notification.error({
    ...
    const handleUploadFileLogo = async ({ file, onSuccess, onError }: any) => {
        const res = await callUploadSingleFile(file, "cinema");
        if (res && res.data) {
            setDataLogo([{
    ...
    return (
        <>
            {openModal &&
                <>
                    <ModalForm
                        title={<>{dataInit?._id ? "Cập nhật Cinema" : "Tạo mới Cinema"}</>}
                        open={openModal}
                        modalProps={{
                            ...
                            keyboard: false,
                            maskClosable: false,
                            className: `modal-cinema ${animation}`,
                            rootClassName: `modal-cinema-root ${animation}`
                        }}
                        scrollToFirstError={true}
                        preserve={false}
                        form={form}
                    onFinish={submitCinema}
                                                ...
                                                [
                                                    {
                                                        ...
                                                        name: dataInit?.logo ?? "",
                                                        status: 'done',
                                                        url: `${import.meta.env.VITE_BACKEND_URL}/images/cinema/${dataInit?.logo}`,
                                                    }
                                                ] : []
                                        }
